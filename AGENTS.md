...
- **Transforming an RGB565 image needs `LV_DRAW_SW_SUPPORT_RGB565A8` as well as `LV_DRAW_SW_SUPPORT_RGB565`.** LVGL compiles its `LV_COLOR_FORMAT_RGB565` case in `lv_draw_sw_transform.c` only when both are set (it routes to `transform_rgb565a8(..., has_alpha=false)`), and ESPHome enables the second only for images declared transparent. Left opaque, `bg_image`'s `zoom` drew **nothing** and LVGL logged `Color format 0x12 is not enabled` once per block — 270 lines per full-screen repaint, ~700 ms of main loop, enough to push Sendspin past its buffer and resync. Hence `transparency: alpha_channel` on `artwork_slot`, which costs an alpha byte per pixel and fixes all of it: 0 warnings, artwork redraw down from 749 ms to 208 ms. Do not "clean up" that option because the artwork is opaque JPEG.
- **On the panel, the SPI flush is the frame-rate limit, not the DSPi link.** ESPHome's ESP-IDF SPI delegate uses `spi_device_polling_start`/`_end`, a blocking busy-wait on the main loop, so repainted pixels are main-loop milliseconds: at 40 MHz in RGB565 the whole 320x240 screen costs ~31 ms and the spectrum's 272x140 bar area ~15 ms. A spectrum band frame, by contrast, is 89 bytes — about 2 ms at 460800 baud, with a full round trip fitting inside one 16 ms loop iteration. Optimise the redraw area, not the polling.
- **Never draw a bar graph as one widget per band.** LVGL tracks at most `LV_INV_BUF_SIZE` (32) invalid areas and invalidates the whole screen once that overflows. With 34 bands, individual widgets repaint all 320x240 every frame and roughly halve the rate; a single `canvas` is one invalidation of a region we choose.
- **The audio path survives a busy loop.** I2S and Sendspin run in their own FreeRTOS tasks behind a 500 ms buffer, so blocking main-loop work slows the UI and the UART, not playback.
